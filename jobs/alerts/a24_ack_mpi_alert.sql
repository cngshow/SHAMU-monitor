...
BEGIN
  select sum(nvl(case when A.SENDING_SITE = v_mpi_site and A.EVENT_TYPE = v_adt_a24 then 1 else 0 end,0)) as mpi_a24,
         sum(nvl(case when A.SENDING_SITE = v_va_site and A.EVENT_TYPE = v_ack_a24 then 1 else 0 end,0)) as vha_ack
  ...
  and    a.sending_site in (v_va_site,v_mpi_site)
  and    a.receiving_site in (v_va_site,v_mpi_site);
...
--n_ack_cnt := 1;
    ...
    if (n_a24_cnt = 0) then
       ...
       v_subject := 'VA to MPI A24_ACK Alert!';
       v_msg := 'VA has not received any A24s from MPI. The status reported is based on the last successful run.';
    else
       if (n_ack_cnt = 0) then
          v_status := 'RED';
          v_subject := 'VA to MPI A24_ACK Alert!';
          v_msg := 'While VA has received A24 messages from MPI, VA CHDR has not sent any acknowledgements (A24_ACKs) back to MPI.';
       else
          if (round((n_ack_cnt/n_a24_cnt)*100, 0) < v_pct_ratio) then
            v_status := 'RED';
            v_subject := 'VA to MPI A24_ACK Alert!';
            v_msg := 'While VA has received A24 messages from MPI, VA CHDR has not sent enough acknowledgements (A24_ACKs) back to MPI based on the alert configuration. This alert goes RED when the response ratio is less than ' || to_char(v_pct_ratio) || '%. The current response ratio is ' || round((n_ack_cnt/n_a24_cnt)*100, 0) || '%.';
          else
            v_status := 'GREEN';
            v_subject := 'VA to MPI A24_ACK Restored!';
            v_msg := 'VA CHDR has successfully sent A24_ACK messages to MPI.';
          end if;
       end if;
    end if;
    ...
    DBMS_OUTPUT.put_line ('A24s Sent By MPI To VA = ' || n_a24_cnt || '<br/>');
    DBMS_OUTPUT.put_line ('A24 ACKs Sent By VA To MPI = ' || n_ack_cnt || '<br/>');

    v_introscope_data := 'MPI_TO_VA_A24=' || to_char(n_a24_cnt) || ';VA_TO_MPI_ACK=' || to_char(n_ack_cnt);
    DBMS_OUTPUT.put_line ('<span class="status">');
